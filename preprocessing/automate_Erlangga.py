...
def load_data():
    """Load raw Titanic dataset dari URL (Sesuai Notebook)"""
    print("Loading Titanic data from URL...")
...
def preprocess_data(df):
    ...
    # 3. Encoding manual (Sesuai Notebook)
    if 'Sex' in df_clean.columns:
        df_clean['Sex'] = df_clean['Sex'].map({'male': 0, 'female': 1})
    if 'Embarked' in df_clean.columns:
        df_clean['Embarked'] = df_clean['Embarked'].map({'S': 0, 'C': 1, 'Q': 2})

    # Drop sisa baris yang masih null (jika ada setelah mapping)
